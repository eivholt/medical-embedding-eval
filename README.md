...
1. Configure Azure OpenAI credentials by copying `.env.example` to `.env` and filling in the endpoint, API key, and deployment names.
    ...
    - Optional: install `torch` and `transformers` and (if needed) set `HUGGINGFACE_DEVICE` plus `HF_EMBED_MODEL` / `HF_E5_EMBED_MODEL` to try Hugging Face checkpoints such as `NbAiLab/nb-bert-large` or `intfloat/multilingual-e5-large-instruct` on your local GPU.
